  ...
  try {
    const stats = await pool.query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM dca_orders WHERE is_active = true) as active_orders,
        (SELECT COUNT(*) FROM dca_orders) as total_orders,
        ...
        (SELECT COUNT(*) FROM gateways WHERE is_active = true) as active_gateways,
        (SELECT COUNT(*) FROM trading_pairs WHERE is_active = true) as active_pairs
    `);
...
app.get('/api/admin/users', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT u.*, 
             COUNT(DISTINCT dca.id) as total_orders,
             COUNT(DISTINCT dca.id) FILTER (WHERE dca.is_active = true) as active_orders,
             COUNT(DISTINCT t.id) as total_transactions,
             COALESCE(SUM(t.amount) FILTER (WHERE t.status = 'completed'), 0) as total_volume
      FROM users u
      LEFT JOIN dca_orders dca ON u.id = dca.user_id
...
app.delete('/api/admin/users/:id', async (req, res) => {
  try {
    // Primero eliminar transacciones y órdenes relacionadas
    await pool.query('DELETE FROM transactions WHERE user_id = $1', [req.params.id]);
    await pool.query('DELETE FROM dca_orders WHERE user_id = $1', [req.params.id]);
...
// --- DCA ORDERS (Admin) ---
app.get('/api/admin/dca-orders', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT dca.*, u.wallet_address,
             (SELECT COUNT(*) FROM transactions WHERE dca_order_id = dca.id) as execution_count,
             (SELECT COUNT(*) FROM transactions WHERE dca_order_id = dca.id AND status = 'completed') as successful_executions
      FROM dca_orders dca
      JOIN users u ON dca.user_id = u.id
    ...
    console.error('Error dca-orders:', error);
    res.status(500).json({ error: 'Error al obtener órdenes DCA' });
  }
});

app.get('/api/admin/dca-orders/:id', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT dca.*, u.wallet_address
      FROM dca_orders dca
...
app.patch('/api/admin/dca-orders/:id/toggle', async (req, res) => {
  try {
    const result = await pool.query(
      'UPDATE dca_orders SET is_active = NOT is_active, updated_at = NOW() WHERE id = $1 RETURNING *',
      [req.params.id]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Orden no encontrada' });
    res.json({ 
      success: true, 
      order: result.rows[0],
      message: result.rows[0].is_active ? 'Orden activada' : 'Orden pausada'
    });
  } catch (error) {
    res.status(500).json({ error: 'Error al cambiar estado de la orden' });
  }
});

app.delete('/api/admin/dca-orders/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM transactions WHERE dca_order_id = $1', [req.params.id]);
    const result = await pool.query('DELETE FROM dca_orders WHERE id = $1 RETURNING *', [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Orden no encontrada' });
    res.json({ message: 'Orden y transacciones relacionadas eliminadas' });
  } catch (error) {
    res.status(500).json({ error: 'Error al eliminar orden' });
  }
});

// Simulate DCA execution (for testing)
app.post('/api/admin/dca-orders/:id/execute', async (req, res) => {
  try {
    const orderResult = await pool.query(`
      SELECT dca.*, u.wallet_address FROM dca_orders dca
      JOIN users u ON dca.user_id = u.id WHERE dca.id = $1
    `, [req.params.id]);
    
    if (orderResult.rows.length === 0) return res.status(404).json({ error: 'Orden no encontrada' });
    
    const order = orderResult.rows[0];
    const txHash = `klv_sim_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    await pool.query(
      `INSERT INTO transactions (dca_order_id, user_id, tx_hash, amount, token_from, token_to, status, gas_used)
       VALUES ($1, $2, $3, $4, $5, $6, 'completed', $7)`,
      [order.id, order.user_id, txHash, order.amount, order.token_from, order.token_to, 0.001]
    );
    ...
    res.json({ 
      success: true, 
      message: 'Ejecución simulada completada',
      tx_hash: txHash,
      next_execution: nextExecution
    });
  } catch (error) {
    console.error('Error simulate:', error);
    res.status(500).json({ error: 'Error al simular ejecución' });
  }
});
...
app.get('/api/admin/transactions', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT t.*, u.wallet_address, dca.token_from as order_token_from, dca.token_to as order_token_to
      FROM transactions t
      JOIN users u ON t.user_id = u.id
      LEFT JOIN dca_orders dca ON t.dca_order_id = dca.id
